...
\todo[inline]{Consistency check: do we use "GPU's memory" or "GPU memory".  client side vs. client-side? Others?}



\begin{abstract}
Titan, the Cray XK7 at Oak Ridge National Laboratory, has 18,688 compute nodes. Each node consists of a 16-core AMD CPU, a NVIDIA GPU, and 32GB DRAM. In addition, there is another 6GB of GDDR DRAM on each GPU card. Not all the applications that run on Titan make use of all a node's resources. For applications that are not otherwise using the GPU, this paper discusses a technique for using the GPU's RAM as a large write-back cache to improve the application's \emph{perceived} file write performance.
\end{abstract}
...
\input{03_analysis}
\input{05_conclusion}
\input{04_futurework} % deliberately changing the order here.  Don't feel like changing the filenames, though.
