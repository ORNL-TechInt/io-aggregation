...
static uint64_t allocateGPUMem( void **devPtr, uint64_t reqLen)
{
    uint64_t ret = 0;
    size_t freeMem, totalMem;
    CUDA_CHECK_RETURN( cudaMemGetInfo( &freeMem, &totalMem));
...
//             << "MB.  (" << (freeMem / (1024*1024)) << "MB free.)" << endl;
    ...
    // We don't ever seem to be able to allocate all of what's reported in
    // freeMem, so attempt to allocate up to (freeMem - 1MB)
...
static void handleWriteRequest( const IoMsg *rx, cci_connection_t *conn)
{
    ...
    uint32_t len;
    void *memPtr;
    len = allocateGPUMem( &memPtr, rx->writeRequest.len);
